...
- For browsing, we used the `LIKE` predicate for the prefix
   - To browse by a character prefix, in [MovieList.java](https://github.com/uci-jherold2-2025spring-cs122b/2025-spring-cs-122b-aa-rs/blob/09da8cb8b2425b74198b817f802ecddd13cdfb61/src/MovieListServlet.java#L107), we used `LIKE LOWER(?)`, where ? would be replaced with the concatenation of the character parameter and '%'
      - Full clause used: `LEFT(LOWER(m.title), 1) LIKE LOWER(?)` to check only the first letter of the title
