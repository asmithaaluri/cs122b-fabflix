...
@WebServlet(name = "movies.MovieListServlet", urlPatterns = "/api/movies")
public class MovieListServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        Claims validateClaims = JwtUtil.validateToken(JwtUtil.getCookieValue(request, "jwtToken"));
        if (validateClaims != null) {
            claims.putAll(validateClaims);
        }
        ...
        if (genre != null) { // new genre to browse by
            claims.put("genre", genre);
            // reset all other filters
            claims.remove("prefix");
            claims.remove("title");
            claims.remove("year");
            claims.remove("director");
            claims.remove("star");
            claims.put("page", PAGE_ONE);
        } else if (prefix != null) { // new prefix to browse by
            claims.put("prefix", prefix);
            // reset all other filters
            claims.remove("genre");
            claims.remove("title");
            claims.remove("year");
            ...
            genre = (String) claims.get("genre");
            prefix = (String) claims.get("prefix");
            ptitle = (String) claims.get("title");
            pyear = (String) claims.get("year");
            pdirector = (String) claims.get("director");
            pstar = (String) claims.get("star");
        }
        if (sort != null) {
            claims.put("sort", sort);
            claims.put("page", PAGE_ONE);
            claims.put("moviesPerPage", movies);
        } else {
            sort = (String) claims.get("sort");
            movies = (String) claims.get("moviesPerPage");
        }
        if (page != null) {
            claims.put("page", page);
        } else {
            page = (String) claims.get("page");
        }
        ...
        try (Connection conn = dataSource.getConnection()) {
            ...
            String newToken = JwtUtil.generateToken(validateClaims.getSubject(), claims); // I hardcoded "user."
